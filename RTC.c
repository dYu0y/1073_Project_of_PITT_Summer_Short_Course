...
void RTC_Set_Time(uint32_t Hour, uint32_t Minute, uint32_t Second){
   //RTC->TR = xxxx
   int time = (Hour << 16) | (Minute << 8) | Second;
...
void RTC_Read_Time(uint32_t *Hour, uint32_t *Minute, uint32_t *Second){
   //xxxx = RTC->TR
   uint32_t data = RTC->TR;
   *Hour = ((data >> 20) & 0x3) * 10 +
                  ((data >> 16) & 0xf);

    *Minute = ((data >> 12) & 0x7) * 10 +
                  ((data >> 8) & 0xf);

    *Second = ((data >> 4) & 0x7) * 10 +
                  ((data >> 0) & 0xf);
}
...
void RTC_Init(uint32_t Hour, uint32_t Minute, uint32_t Second){
   ...
   // Configure the Time 
   RTC_Set_Time(Hour, Minute, Second); // // Set Time: 11:59:55 AM
